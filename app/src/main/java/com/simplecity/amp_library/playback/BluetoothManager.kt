...
class BluetoothManager(
    ...
    private fun handlePauseOnDisconnect(action: String, extras: Bundle) {
        when (action) {
            ...
            BluetoothHeadset.ACTION_AUDIO_STATE_CHANGED -> {
                ...
                if (state == BluetoothHeadset.STATE_AUDIO_DISCONNECTED && prev == BluetoothHeadset.STATE_AUDIO_CONNECTED) {
    ...
    private fun wasDisconnected(state: Int, previousState: Int): Boolean {
        return (state == BluetoothA2dp.STATE_DISCONNECTED || state == BluetoothA2dp.STATE_DISCONNECTING) &&
